...
async function getClubSchedules() {
    ...
    const club_schedule_query = `SELECT cs.id,
                                    ...
                                    UNIX_TIMESTAMP(CONVERT_TZ(\`from\`,c.time_zone,@@session.time_zone)) DIV 1 AS from_ms,
                                    UNIX_TIMESTAMP(CONVERT_TZ(\`to\`,c.time_zone,@@session.time_zone)) DIV 1 AS to_ms,
